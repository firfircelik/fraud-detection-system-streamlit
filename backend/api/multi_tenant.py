...
class ResourceManager:
    ...
    def _reset_resource_if_needed(self, resource: TenantResource):
        ...
        if resource.period == "minute":
            ...
            if (
                resource.last_reset.month != now.month
                or resource.last_reset.year != now.year
            ):
                resource.current_usage = 0
                resource.last_reset = now
        ...
        with self.lock:
            ...
            for resource_key, resource in self.tenant_resources[tenant_id].items():
                ...
                usage[resource_key] = {
                    ...
                    "period": resource.period,
                    "utilization_percent": (
                        (resource.current_usage / resource.limit) * 100
                        if resource.limit > 0
                        else 0
                    ),
...
class FeatureManager:
    ...
    def _check_feature_dependencies(
        self, feature: str, available_features: Set[str]
    ) -> bool:
    ...
    def enable_feature(self, tenant_id: str, feature: str) -> bool:
        ...
        # Check dependencies
        if not self._check_feature_dependencies(
            feature, self.tenant_features[tenant_id]
        ):
            return False
...
class TenantManager:
    ...
    def create_tenant(self, tenant_data: Dict[str, Any]) -> str:
        ...
        isolation_level = IsolationLevel(
            tenant_data.get(
                "isolation_level", self._get_default_isolation_level(tier).value
            )
        )
        ...
        tenant_config = TenantConfig(
            ...
            subscription_expires_at=(
                datetime.fromisoformat(tenant_data["subscription_expires_at"])
                if tenant_data.get("subscription_expires_at")
                else None
            ),
        )
    ...
    def _save_tenant(self, tenant_config: TenantConfig):
        ...
        config_data = {
            ...
            "subscription_expires_at": (
                tenant_config.subscription_expires_at.isoformat()
                if tenant_config.subscription_expires_at
                else None
            ),
        }
    ...
    def check_tenant_access(
        self, tenant_id: str, user_id: str, required_permission: str = None
    ) -> bool:
    ...
    def get_tenant_summary(self, tenant_id: str) -> Dict[str, Any]:
        ...
        # Recent metrics
        recent_metrics = (
            self.tenant_metrics.get(tenant_id, [])[-1:]
...
def main():
    ...
    # Check feature
    has_ml = tenant_manager.feature_manager.is_feature_enabled(
        tenant_id, "ml_fraud_detection"
    )
