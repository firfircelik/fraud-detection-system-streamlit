...
class RedisManager:
    ...
    def __init__(self):
        self.redis_url = os.getenv('REDIS_URL', 'redis://:RedisStack2024!@localhost:6379')
    ...
    async def cache_user_features(self, user_id: str, features: Dict[str, float], 
                                version: str = "1.0", ttl: int = None) -> bool:
        ...
        try:
            ...
            await self.async_redis.setex(
                cache_key,
                ttl,
                json.dumps(asdict(cached_features), default=str)
            )
    ...
    async def cache_merchant_features(self, merchant_id: str, features: Dict[str, float], ttl: int = None) -> bool:
    ...
    async def get_merchant_features(self, merchant_id: str) -> Optional[Dict[str, float]]:
    ...
    async def cache_fraud_score(self, transaction_id: str, fraud_score: float, 
                              risk_level: str, decision: str, ttl: int = None) -> bool:
    ...
    async def cache_session(self, session_id: str, session_data: Dict[str, Any], ttl: int = None) -> bool:
