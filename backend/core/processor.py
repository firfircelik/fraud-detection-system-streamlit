...
class CSVFraudProcessor:
    ...
    def load_csv_file(
        self, filename: str, sample_size: Optional[int] = None
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if file_size_mb > 100:
                logger.info(
                    f"Large file detected ({file_size_mb:.1f}MB). Using optimized chunked reading..."
                )
    ...
    def standardize_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        required_columns = {
            ...
            "merchant_id": lambda: [
                f"merchant_{i%1000:04d}" for i in range(len(df_renamed))
            ],
            "category": lambda: np.random.choice(
                ["grocery", "electronics", "gas", "restaurant"], len(df_renamed)
            ),
            "timestamp": lambda: [
                datetime.now().isoformat() for _ in range(len(df_renamed))
            ],
    ...
    def calculate_fraud_score(self, row: pd.Series) -> Tuple[float, str, List[str]]:
        ...
        try:
            ...
            hour = timestamp.hour
            if (
                hour >= self.fraud_rules["late_hour_start"]
                or hour <= self.fraud_rules["early_hour_end"]
            ):
                score += 0.2
    ...
    def generate_summary_report(self, df_processed: pd.DataFrame) -> Dict:
        ...
        risk_factor_counts = (
            pd.Series(all_risk_factors).value_counts().head(10).to_dict()
        )
        ...
        summary = {
            ...
            "fraud_rate": len(df_processed[df_processed["decision"] == "DECLINED"])
            / total_transactions,
        }
    ...
    def save_results(
        self, df_processed: pd.DataFrame, summary: Dict, filename_prefix: str
    ) -> Tuple[str, str]:
...
if __name__ == "__main__":
    ...
    if files:
        ...
        df_processed, summary, csv_path, json_path = processor.process_file(
            sample_file, sample_size=1000  # Process only 1000 rows for demo
        )
