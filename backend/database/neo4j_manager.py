...
class GraphNode:
    ...
    node_id: str
    node_type: (
        str  # 'User', 'Merchant', 'Transaction', 'Device', 'Location', 'IPAddress'
    )
...
class Neo4jManager:
    ...
    def __init__(
        ...
        Args:
            uri: Neo4j database URI (defaults to .env NEO4J_URI)
            username: Database username (defaults to .env NEO4J_USERNAME)
            password: Database password (defaults to .env NEO4J_PASSWORD)
            database: Database name (defaults to .env NEO4J_DATABASE)
        """
    ...
    def create_transaction_node(self, transaction_data: Dict[str, Any]) -> bool:
        ...
        try:
            with self.driver.session() as session:
                ...
                logger.info(
                    f"Created transaction node with relationships: {transaction_id}"
                )
                return True
    ...
    def create_device_relationship(
        ...
        try:
            with self.driver.session() as session:
                ...
                logger.info(
                    f"Created/updated device relationship: {user_id} -> {device_id}"
                )
                return True
    ...
    def detect_device_sharing_rings(
        self, min_users: int = 2, min_risk_score: float = 0.5
    ) -> List[FraudRing]:
        ...
        try:
            with self.driver.session() as session:
                ...
                result = session.run(
                    query, {"min_users": min_users, "min_risk_score": min_risk_score}
                )
                ...
                for record in result:
                    ...
                    ring = FraudRing(
                        ...
                        confidence=min(
                            record["avg_risk_score"] * record["user_count"] / 10, 1.0
                        ),
    ...
    def detect_velocity_rings(
        self, time_window_minutes: int = 60, min_transactions: int = 5
    ) -> List[FraudRing]:
            ...
            with self.driver.session() as session:
                ...
                for record in result:
                    ring = FraudRing(
                        ...
                        confidence=min(
                            record["merchant_avg_fraud_score"]
                            * record["user_count"]
                            / 5,
                            1.0,
                        ),
            ...
            with self.driver.session() as session:
                ...
                for record in result:
                    ring = FraudRing(
                        ...
                        confidence=min(
                            record["avg_risk_score"] * record["member_count"] / 8, 1.0
                        ),
                        detected_at=datetime.now(),
                    )
                    fraud_rings.append(ring)
                ...
                logger.info(
                    f"Detected {len(fraud_rings)} community-based fraud rings using {algorithm}"
                )
                return fraud_rings
    ...
    def get_entity_network(
        self, entity_id: str, max_depth: int = 2, min_relationship_strength: float = 0.3
    ) -> Dict[str, Any]:
            ...
            with self.driver.session() as session:
                ...
                if record:
                    return {
                        ...
                        "fraud_rate": record["high_risk_transactions"]
                        / max(record["total_transactions"], 1),
                        ...
                        "avg_processing_time": round(
                            record["avg_processing_time"] or 0, 2
                        ),
    ...
    def get_top_risky_entities(
        self, entity_type: str = "User", limit: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        try:
            with self.driver.session() as session:
                ...
                logger.info(
                    f"Retrieved {len(entities)} top risky {entity_type.lower()}s"
                )
                return entities
...
if __name__ == "__main__":
    ...
    try:
        with Neo4jManager() as neo4j:
            ...
            device_rings = neo4j.detect_device_sharing_rings(
                min_users=2, min_risk_score=0.5
            )
            print(f"Detected {len(device_rings)} device sharing rings")

            velocity_rings = neo4j.detect_velocity_rings(
                time_window_minutes=60, min_transactions=3
            )
