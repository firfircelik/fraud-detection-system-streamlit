...
class LightweightModelPerformanceTracker:
    ...
    def _update_performance_metrics(self, model_name: str) -> None:
        ...
        try:
            ...
            # Calculate metrics
            predictions_with_actual = [p for p in recent_predictions if p['actual'] is not None]
            
            if len(predictions_with_actual) < 10:
                return
            
            y_true = [p['actual'] for p in predictions_with_actual]
            y_pred = [1 if p['prediction'] > 0.5 else 0 for p in predictions_with_actual]
            ...
            if not metrics.is_healthy:
                logger.warning(f"Performance degradation detected for {model_name}: accuracy={accuracy:.3f}, drift={drift_score:.3f}")
    ...
    def get_model_performance(self, model_name: str) -> Optional[ModelPerformanceMetrics]:
...
class LightweightEnsembleManager:
    ...
    def predict_single_model(self, model_name: str, features: np.ndarray) -> ModelPrediction:
        ...
        try:
            ...
            confidence = abs(fraud_probability - 0.5) * 2  # Distance from 0.5, scaled to 0-1
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                feature_importance = {f'feature_{i}': np.random.random() 
    ...
    def predict_ensemble(self, features: np.ndarray, 
                        transaction_id: str = None) -> EnsemblePrediction:
        ...
        active_models = [name for name, info in self.models.items() 
    ...
    def _calculate_ensemble_prediction(self, predictions: List[ModelPrediction], 
                                     transaction_id: str = None) -> EnsemblePrediction:
    ...
    def _dynamic_ensemble(self, predictions: List[ModelPrediction]) -> float:
        ...
        for pred in predictions:
            performance = self.performance_tracker.get_model_performance(pred.model_name)
    ...
    def _determine_risk_and_decision(self, fraud_probability: float, 
    ...
    def _aggregate_feature_importance(self, predictions: List[ModelPrediction]) -> Dict[str, float]:
    ...
    def _generate_explanation(self, predictions: List[ModelPrediction], 
                            ensemble_prob: float) -> Dict[str, Any]:
    ...
    def _get_top_contributing_models(self, predictions: List[ModelPrediction]) -> List[Dict]:
    ...
    def _analyze_confidence(self, predictions: List[ModelPrediction]) -> Dict[str, float]:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"✅ Ensemble initialized: {status['active_models']}/{status['total_models']} active models")
        ...
        for pred in prediction.individual_predictions:
            print(f"   {pred.model_name}: {pred.fraud_probability:.4f} "
