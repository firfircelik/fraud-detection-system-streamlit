...
class SHAPExplainer:
    ...
    def initialize_explainers(
        self, models: Dict, background_data: np.ndarray, feature_names: List[str] = None
    ):
        ...
        for model_name, model_info in models.items():
            try:
                ...
                logger.error(
                    f"Failed to initialize SHAP explainer for {model_name}: {e}"
                )

    def explain_prediction(
        self, model_name: str, features: np.ndarray, transaction_id: str = None
    ) -> Dict[str, Any]:
                ...
                top_features.append(
                    {
                        ...
                        "contribution_type": (
                            "positive" if shap_values[idx] > 0 else "negative"
                        ),
                    }
                )
    ...
    def _generate_explanation_summary(self, shap_values: np.ndarray) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "most_important_feature": {
                "name": (
                    self.feature_names[most_important_idx]
                    if most_important_idx < len(self.feature_names)
                    else f"feature_{most_important_idx}"
                ),
                "value": float(shap_values[most_important_idx]),
                "impact": (
                    "increases fraud risk"
                    if shap_values[most_important_idx] > 0
                    else "decreases fraud risk"
                ),
            },
            "explanation_text": self._generate_explanation_text(
                shap_values, positive_contrib, negative_contrib
            ),
        }
    ...
    def _generate_explanation_text(
        self, shap_values: np.ndarray, positive_contrib: float, negative_contrib: float
    ) -> str:
        ...
        if positive_features:
            ...
            explanation_parts.append(
                f"Features increasing fraud risk: {', '.join(feature_names)}"
            )
        ...
        if negative_features:
            ...
            explanation_parts.append(
                f"Features decreasing fraud risk: {', '.join(feature_names)}"
            )
        ...
        if positive_contrib > abs(negative_contrib):
            ...
            explanation_parts.append(
                "Overall prediction leans towards legitimate transaction"
            )
...
class EnsembleSHAPExplainer:
    ...
    def add_explainer(
        self, model_name: str, explainer: SHAPExplainer, weight: float = 1.0
    ):
    ...
    def explain_ensemble_prediction(
        ...
        for model_name, explainer in self.individual_explainers.items():
            explanation = explainer.explain_prediction(
                model_name, features, transaction_id
            )
        ...
        ensemble_explanation = {
            ...
            "aggregated_contributions": self._aggregate_feature_contributions(
                individual_explanations
            ),
            "consensus_analysis": self._analyze_model_consensus(
                individual_explanations
            ),
            "timestamp": datetime.now().isoformat(),
        }
    ...
    def _aggregate_feature_contributions(
        self, individual_explanations: Dict
    ) -> Dict[str, float]:
...
if __name__ == "__main__":
    ...
    try:
        ...
        explanation = explainer.explain_prediction(
            "test_model", test_features, "test_tx_001"
        )

        print("✅ SHAP explanation generated successfully!")
        print(f"   Top features: {len(explanation.get('top_features', []))}")
        print(
            f"   Feature contributions: {len(explanation.get('feature_contributions', {}))}"
        )
        print(
            f"   Explanation summary available: {'explanation_summary' in explanation}"
        )
