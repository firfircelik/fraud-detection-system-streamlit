...
class AdvancedFeatureEngineer:
    ...
    def create_temporal_features(self, df: pd.DataFrame, timestamp_col: str = 'timestamp') -> pd.DataFrame:
        ...
        # Time since features
        df['days_since_epoch'] = (df[timestamp_col] - pd.Timestamp('1970-01-01')).dt.days
        df['seconds_since_midnight'] = df['hour'] * 3600 + df[timestamp_col].dt.minute * 60 + df[timestamp_col].dt.second
        
        return df
    
    def create_velocity_features(self, df: pd.DataFrame, user_col: str = 'user_id', 
        ...
        # Time differences
        df['time_diff_prev'] = df.groupby(user_col)[timestamp_col].diff().dt.total_seconds()
        df['time_diff_next'] = df.groupby(user_col)[timestamp_col].diff(-1).dt.total_seconds().abs()
        
        # Velocity features (transactions per time window)
        for window in ['1H', '6H', '1D', '7D', '30D']:
            df[f'tx_count_{window}'] = df.groupby(user_col)[timestamp_col].rolling(window).count().values
            df[f'tx_velocity_{window}'] = df[f'tx_count_{window}'] / pd.Timedelta(window).total_seconds() * 3600
        
        # Amount velocity
        if 'amount' in df.columns:
            for window in ['1H', '6H', '1D']:
                df[f'amount_sum_{window}'] = df.groupby(user_col)['amount'].rolling(window).sum().values
                df[f'amount_velocity_{window}'] = df[f'amount_sum_{window}'] / pd.Timedelta(window).total_seconds() * 3600
        
        # Rapid fire detection
        df['is_rapid_fire'] = (df['time_diff_prev'] < 300).astype(int)  # Less than 5 minutes
        df['rapid_fire_count'] = df.groupby(user_col)['is_rapid_fire'].rolling(window=10).sum().values
    ...
    def create_network_features(self, df: pd.DataFrame, user_col: str = 'user_id', 
                              merchant_col: str = 'merchant_id') -> pd.DataFrame:
        ...
        df['user_pagerank'] = df[user_col].map(
            lambda x: pagerank.get(f"user_{x}", 0)
        )
    ...
    def create_statistical_features(self, df: pd.DataFrame, amount_col: str = 'amount') -> pd.DataFrame:
        ...
        for window in [5, 10, 20, 50]:
            df[f'{amount_col}_rolling_mean_{window}'] = df[amount_col].rolling(window).mean()
            df[f'{amount_col}_rolling_std_{window}'] = df[amount_col].rolling(window).std()
            df[f'{amount_col}_rolling_min_{window}'] = df[amount_col].rolling(window).min()
            df[f'{amount_col}_rolling_max_{window}'] = df[amount_col].rolling(window).max()
            df[f'{amount_col}_rolling_median_{window}'] = df[amount_col].rolling(window).median()
            
            # Percentiles
            df[f'{amount_col}_rolling_q25_{window}'] = df[amount_col].rolling(window).quantile(0.25)
            df[f'{amount_col}_rolling_q75_{window}'] = df[amount_col].rolling(window).quantile(0.75)
        ...
        # Trend features
        df[f'{amount_col}_trend_5'] = df[amount_col].rolling(5).apply(
            lambda x: np.polyfit(range(len(x)), x, 1)[0] if len(x) == 5 else 0
        )
        df[f'{amount_col}_trend_10'] = df[amount_col].rolling(10).apply(
            lambda x: np.polyfit(range(len(x)), x, 1)[0] if len(x) == 10 else 0
        )
        
        return df
    
    def create_clustering_features(self, df: pd.DataFrame, feature_cols: List[str]) -> pd.DataFrame:
        ...
        for n_clusters in [5, 10, 20]:
            ...
            df[f'kmeans_distance_ratio_{n_clusters}'] = (
                df[f'kmeans_distance_{n_clusters}'] / np.mean(distances, axis=1)
            )
    ...
    def create_dimensionality_reduction_features(self, df: pd.DataFrame, 
        ...
        # ICA
        ica = FastICA(n_components=min(5, len(numeric_features.columns)), random_state=42)
        ...
        # SVD
        svd = TruncatedSVD(n_components=min(5, len(numeric_features.columns)), random_state=42)
    ...
    def create_interaction_features(self, df: pd.DataFrame, 
                                  feature_cols: List[str], max_interactions: int = 20) -> pd.DataFrame:
        """Feature interactions"""
        
        df = df.copy()
        
        # Select numeric features
        numeric_features = [col for col in feature_cols if col in df.columns and 
    ...
    def create_frequency_encoding(self, df: pd.DataFrame, categorical_cols: List[str]) -> pd.DataFrame:
        ...
        for col in categorical_cols:
            if col in df.columns:
                ...
                # Rank encoding
                rank_map = df[col].value_counts().rank(method='dense', ascending=False).to_dict()
        ...
        for col in categorical_cols:
            if col in df.columns:
                ...
                for train_idx, val_idx in kf.split(df):
                    ...
                    df.loc[val_idx, f'{col}_target_encoded'] = df.loc[val_idx, col].map(target_mean)
    ...
    def engineer_all_features(self, df: pd.DataFrame, user_col: str = 'user_id',
                            merchant_col: str = 'merchant_id', timestamp_col: str = 'timestamp',
                            amount_col: str = 'amount', target_col: str = 'is_fraud') -> pd.DataFrame:
        ...
        categorical_cols = [merchant_col, 'category'] if 'category' in df.columns else [merchant_col]
        ...
        df = self.create_clustering_features(df, numeric_cols[:20])  # Limit for performance
        ...
        important_cols = [amount_col, 'hour', 'day_of_week', 'amount_count', 'merchant_id_nunique']
...
if __name__ == "__main__":
    ...
    sample_data = pd.DataFrame({
        ...
        'category': np.random.choice(['grocery', 'electronics', 'gas', 'restaurant'], n_samples),
