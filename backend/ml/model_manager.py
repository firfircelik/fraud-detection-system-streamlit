...
class ModelManager:
    ...
    async def create_model_tables(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_model_performance_model_date ON ml_model_performance (model_id, evaluation_date DESC)"
            )
        ...
        try:
            ...
            if self.use_s3:
                ...
                storage_path = os.path.join(
                    self.model_storage_path, f"{model_id}{model_extension}"
                )
            ...
            async with self.pool.acquire() as conn:
                ...
                """,
                    (
                        ...
                        model_metadata.training_config,
                        model_metadata.performance_metrics,
                        model_metadata.model_size_bytes,
                        model_metadata.storage_path,
                        model_metadata.deployment_status,
        ...
        try:
            ...
            if self.use_s3:
                ...
                response = self.s3_client.get_object(
                    Bucket=self.s3_bucket, Key=model_row["storage_path"]
                )
    ...
    async def list_models(
        self, model_name: str = None, deployment_status: str = None
    ) -> List[ModelMetadata]:
    ...
    async def record_model_performance(
        ...
        try:
            performance = ModelPerformance(
                model_id=model_id,
                evaluation_date=performance_data.get(
                ...
                avg_inference_time_ms=performance_data.get(
        ...
        try:
            ...
            if self.use_s3:
                self.s3_client.delete_object(
                    Bucket=self.s3_bucket, Key=metadata.storage_path
                )
            else:
            ...
            async with self.pool.acquire() as conn:
                await conn.execute(
                    "DELETE FROM ai_models WHERE model_id = $1", model_id
                )
...
async def main():
    ...
    try:
        ...
        # Record performance
        await model_manager.record_model_performance(
            model_id,
            {
                "accuracy": 0.95,
                "precision": 0.94,
