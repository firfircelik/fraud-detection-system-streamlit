...
class MLTrainer:
    ...
    def load_training_data(self, days_back: int = 30) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        try:
            with self.session_local() as session:
                ...
                feature_columns = [
                    col for col in columns if col not in ["transaction_id", "is_fraud"]
                ]
                ...
                logger.info(
                    f"   - Legitimate cases: {len(y) - y.sum()} ({(1-y.mean())*100:.2f}%)"
                )
    ...
    def train_model(
        ...
        try:
            ...
            try:
                if len(np.unique(y_train)) > 1:  # Ensure both classes are present
                    cv_scores = cross_val_score(
                        model, X_train_scaled, y_train, cv=3, scoring="f1_binary"
                    )
            ...
            model_path = os.path.join(
                self.model_storage_path, f"{model_name}_model.pkl"
            )
            logger.info(f"💾 Saving model to: {model_path}")
            logger.info(
                f"📁 Storage path exists: {os.path.exists(self.model_storage_path)}"
            )
    ...
    def train_all_models(self):
        ...
        if successful_models:
            ...
            logger.info(
                f"   - Best model: {best_model['model_name']} (F1: {best_model['metrics']['f1_score']:.4f})"
            )
