...
class MetricsCollector:
    ...
    def __init__(self):
        self.timescale_dsn = os.getenv(
            "TIMESCALEDB_URL",
            "postgresql://timescale_admin:TimeScale2024!@localhost:5433/fraud_metrics",
        )
    ...
    def setup_prometheus_metrics(self):
        ...
        self.disk_gauge = Gauge(
            "system_disk_percent", "Disk usage percentage", registry=self.registry
        )
        ...
        self.fraud_score_gauge = Gauge(
            "fraud_score_current", "Current fraud score", registry=self.registry
        )

    async def initialize(self):
        """Initialize database connection"""
        self.pool = await asyncpg.create_pool(
            self.timescale_dsn, min_size=5, max_size=20, command_timeout=60
        )
    ...
    async def store_api_metric(self, metric: APIMetric):
        ...
        self.api_request_duration.labels(
            endpoint=metric.endpoint, method=metric.method
    ...
    async def store_fraud_metric(self, metric: FraudMetric):
        ...
        self.fraud_detections_total.labels(
            decision=metric.decision, risk_level=metric.risk_level
        ).inc()
    ...
    async def collect_database_metrics(self) -> List[SystemMetric]:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                for table in tables:
                    result = await conn.fetchrow(
                        f"SELECT pg_total_relation_size('{table}') as size_bytes"
                    )
                    metrics.append(
                        SystemMetric(
                            time=now,
