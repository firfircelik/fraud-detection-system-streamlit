...
class ElasticsearchManager:
    ...
    def __init__(self, hosts: List[str], username: str = None, password: str = None):
        ...
        self.async_client = AsyncElasticsearch(hosts=hosts, basic_auth=auth, verify_certs=False)
    ...
    async def index_transaction(self, transaction: TransactionDocument) -> bool:
        ...
        try:
            ...
            if doc.get('location') and 'lat' in doc['location'] and 'lon' in doc['location']:
                doc['location'] = {
            ...
            logger.error(f"Failed to index transaction {transaction.transaction_id}: {e}")
            return False
    
    async def bulk_index_transactions(self, transactions: List[TransactionDocument]) -> Dict[str, int]:
    ...
    async def search_transactions(self, query: Dict[str, Any], 
                                 size: int = 100,
                                 from_: int = 0,
                                 sort: List[Dict[str, Any]] = None) -> List[SearchResult]:
    ...
    async def geo_search_transactions(self, center_lat: float, center_lon: float,
                                    distance_km: float, 
    ...
    async def complex_fraud_search(self, user_id: str = None,
                                  merchant_id: str = None,
                                  min_amount: float = None,
                                  max_amount: float = None,
                                  min_fraud_score: float = None,
                                  time_range: Tuple[datetime, datetime] = None,
                                  risk_levels: List[str] = None) -> List[SearchResult]:
        ...
        try:
            ...
            if min_fraud_score is not None:
                filter_clauses.append({"range": {"fraud_score": {"gte": min_fraud_score}}})
    ...
    async def fraud_analytics_aggregation(self, time_bucket: str = "1h") -> Dict[str, Any]:
    ...
    async def detect_anomalies(self, documents: List[Dict[str, Any]], 
                              job_type: str = "amount_anomaly") -> List[AnomalyDetectionResult]:
    ...
    async def _detect_amount_anomalies(self, documents: List[Dict[str, Any]]) -> List[AnomalyDetectionResult]:
    ...
    async def _detect_velocity_anomalies(self, documents: List[Dict[str, Any]]) -> List[AnomalyDetectionResult]:
        ...
        for doc in documents:
            ...
            if user_id and timestamp_str:
                try:
                    ...
                    time_bucket = timestamp.replace(minute=(timestamp.minute // 15) * 15, second=0, microsecond=0)
    ...
    async def _detect_pattern_anomalies(self, documents: List[Dict[str, Any]]) -> List[AnomalyDetectionResult]:
