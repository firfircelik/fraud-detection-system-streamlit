...
class FlinkStreamProcessor:
    ...
    async def _assign_to_windows(self, event: TransactionEvent):
        ...
        for window_name, config in self.window_configs.items():
            ...
            if window_type == 'tumbling':
                await self._assign_to_tumbling_window(event, window_name, config, event_time)
            elif window_type == 'sliding':
                await self._assign_to_sliding_window(event, window_name, config, event_time)
            elif window_type == 'session':
                await self._assign_to_session_window(event, window_name, config, event_time)
    
    async def _assign_to_tumbling_window(self, event: TransactionEvent, 
                                       window_name: str, config: Dict, event_time: int):
    ...
    async def _assign_to_sliding_window(self, event: TransactionEvent,
                                      window_name: str, config: Dict, event_time: int):
    ...
    async def _assign_to_session_window(self, event: TransactionEvent,
                                      window_name: str, config: Dict, event_time: int):
        ...
        for window_id, window in self.session_windows.items():
            if window_id.startswith(session_key):
                ...
                last_event_time = max(int(e.timestamp.timestamp() * 1000) for e in window.events)
    ...
    async def _process_window_triggers(self):
        ...
        for window_id, window in list(self.session_windows.items()):
            if window.end_time and current_time >= int(window.end_time.timestamp() * 1000):
                await self._trigger_window_computation(window)
    ...
    async def _trigger_window_computation(self, window: WindowState):
        ...
        try:
            ...
            logger.debug(f"Processed window {window.window_id} with {len(window.events)} events")
    ...
    async def _compute_window_aggregates(self, window: WindowState) -> Dict[str, Any]:
        ...
        aggregates.update({
            'channel_distribution': {channel: channels.count(channel) for channel in set(channels)},
            'type_distribution': {ttype: transaction_types.count(ttype) for ttype in set(transaction_types)}
        })
        
        return aggregates
    
    async def _extract_window_features(self, window: WindowState) -> Optional[FeatureVector]:
        ...
        # Velocity features
        window_duration_minutes = (window.end_time - window.start_time).total_seconds() / 60
        if window_duration_minutes > 0:
            features.update({
                'transactions_per_minute': aggregates.get('event_count', 0) / window_duration_minutes,
                'amount_per_minute': aggregates.get('total_amount', 0) / window_duration_minutes
            })
        ...
        if len(channel_dist) > 1:
            ...
            entropy = -sum((count/total_txns) * np.log2(count/total_txns) 
                          for count in channel_dist.values() if count > 0)
    ...
    async def _create_checkpoint(self):
        ...
        try:
            ...
            checkpoint_data = {
                ...
                'operator_states': {name: op['state'] for name, op in self.operators.items()},
...
async def user_velocity_operator(event: TransactionEvent, state: Dict) -> Optional[Dict]:
    ...
    recent_transactions = [
        t for t in user_state['transactions'] 
...
async def merchant_anomaly_operator(event: TransactionEvent, state: Dict) -> Optional[Dict]:
    ...
    if len(merchant_state['hourly_stats']) > 24:  # At least 24 hours of data
        ...
        if (current_count > avg_count * 3 or  # 3x normal transaction count
            current_amount > avg_amount * 5):  # 5x normal amount
