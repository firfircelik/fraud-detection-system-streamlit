...
class KafkaManager:
    ...
    async def publish_model_prediction(self, prediction_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(f"Published model prediction: {prediction_data['transaction_id']}")
            return True
    ...
    async def publish_user_event(self, user_id: str, event_type: str, event_data: Dict[str, Any]) -> bool:
    ...
    async def create_consumer(self, topic: str, group_id: str, 
                            auto_offset_reset: str = 'latest') -> AIOKafkaConsumer:
    ...
    async def process_transaction_stream(self):
        ...
        async def process_transaction(transaction: TransactionEvent):
            try:
                ...
                logger.info(f"Processed transaction {transaction.transaction_id}: {fraud_score:.3f} ({risk_level})")
    ...
    async def update_processing_state(self, stream_name: str, partition_id: int, offset: int):
    ...
    async def get_processing_state(self, stream_name: str, partition_id: int) -> Optional[int]:
    ...
    async def store_fraud_event(self, alert: FraudAlert):
        ...
        try:
            async with self.pg_pool.acquire() as conn:
                ...
                """, (
                    alert.alert_id, alert.transaction_id, 'fraud_alert',
                    alert.fraud_score, alert.risk_level, alert.decision,
                    alert.model_name, {'risk_factors': alert.risk_factors}
                ))
