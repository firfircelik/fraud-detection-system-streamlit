...
class GeospatialAnalysisEngine:
    ...
    def __init__(self, cache_size: int = 10000):
        ...
        # Geospatial indexing
        self.location_index: Dict[str, List[str]] = defaultdict(list)  # geohash -> location_ids
        self.transaction_index: Dict[str, List[str]] = defaultdict(list)  # geohash -> transaction_ids
    ...
    def add_location(self, latitude: float, longitude: float,
                    location_data: Dict[str, Any] = None) -> str:
        ...
        with self.lock:
            ...
            geo_hash = geohash.encode(latitude, longitude, precision=self.geohash_precision)
    ...
    def add_transaction(self, transaction_data: Dict[str, Any]) -> str:
        ...
        with self.lock:
            ...
            # Add to geospatial index
            geo_hash = geohash.encode(location.latitude, location.longitude, 
        ...
        logger.info(f"Added transaction {transaction_id} with risk score {risk_score:.3f}")
    ...
    def analyze_transaction_risk(self, transaction: GeoTransaction) -> float:
        ...
        total_risk = sum(weights.get(model, 0.1) * score 
                        for model, score in risk_scores.items())
        
        # Normalize to 0-1 range
        final_risk = min(1.0, max(0.0, total_risk))
        
        logger.debug(f"Transaction {transaction.transaction_id} risk breakdown: {risk_scores}, final: {final_risk:.3f}")
    ...
    def _velocity_risk_analysis(self, transaction: GeoTransaction) -> float:
        ...
        for trans in self.transactions.values():
            if (trans.user_id == user_id and 
                trans.timestamp < current_time and
                (previous_transaction is None or trans.timestamp > previous_transaction.timestamp)):
        ...
        time_diff = (current_time - previous_transaction.timestamp).total_seconds() / 3600  # hours
    ...
    def detect_geographic_clusters(self, algorithm: str = 'dbscan',
                                 parameters: Dict[str, Any] = None) -> List[GeoCluster]:
    ...
    def _dbscan_clustering(self, parameters: Dict[str, Any]) -> List[GeoCluster]:
        ...
        for cluster_label in set(labels):
            ...
            cluster_transactions = [
                recent_transactions[i] for i, label in enumerate(labels) 
    ...
    def _grid_based_clustering(self, parameters: Dict[str, Any]) -> List[GeoCluster]:
        ...
        for transaction in recent_transactions:
            # Calculate grid cell
            lat_grid = int(transaction.location.latitude / (grid_size_km / 111.0))  # ~111km per degree
            lon_grid = int(transaction.location.longitude / (grid_size_km / (111.0 * math.cos(math.radians(transaction.location.latitude)))))
    ...
    def _hotspot_clustering(self, parameters: Dict[str, Any]) -> List[GeoCluster]:
        ...
        for transaction in high_risk_transactions:
            ...
            if len(cluster_transactions) >= min_transactions:
                ...
                distances = [
                    self._calculate_distance(center_lat, center_lon, t.location.latitude, t.location.longitude)
                    for t in cluster_transactions
                ]
                ...
                cluster = GeoCluster(
                    ...
                    risk_level='high',
                    confidence_score=np.mean([t.risk_score for t in cluster_transactions]),
                    temporal_pattern=self._analyze_temporal_pattern(cluster_transactions),
                    properties={
                        'algorithm': 'hotspot',
                        ...
                        'transaction_count': len(cluster_transactions),
                        'avg_risk_score': np.mean([t.risk_score for t in cluster_transactions])
                    },
                    created_at=datetime.now(timezone.utc),
    ...
    def _temporal_spatial_clustering(self, parameters: Dict[str, Any]) -> List[GeoCluster]:
        ...
        for transaction in recent_transactions:
            ...
            lon_km = transaction.location.longitude * 111.0 * math.cos(math.radians(transaction.location.latitude))
        ...
        # DBSCAN clustering
        clustering = DBSCAN(
            eps=0.5,  # Adjusted for scaled features
            min_samples=min_samples
        ).fit(features_scaled)
    ...
    def _update_travel_pattern(self, transaction: GeoTransaction):
        ...
        if self._is_suspicious_movement(pattern, transaction):
            suspicious_event = {
                ...
                'location': (transaction.location.latitude, transaction.location.longitude),
    ...
    def _create_travel_pattern(self, user_id: str) -> TravelPattern:
        ...
        if location_counts:
            ...
            home_location = next(
                (loc for loc in locations 
                 if abs(loc.latitude - home_lat) < 0.001 and abs(loc.longitude - home_lon) < 0.001),
                locations[0]
            )
        else:
        ...
        for i in range(1, len(sorted_transactions)):
            ...
            distance = self._calculate_distance(
                prev_trans.location.latitude, prev_trans.location.longitude,
                curr_trans.location.latitude, curr_trans.location.longitude
            )
            
            time_diff = (curr_trans.timestamp - prev_trans.timestamp).total_seconds() / 3600
    ...
    def _is_suspicious_movement(self, pattern: TravelPattern, transaction: GeoTransaction) -> bool:
        ...
        time_diff = (transaction.timestamp - last_transaction.timestamp).total_seconds() / 3600
    ...
    def _get_movement_details(self, pattern: TravelPattern, transaction: GeoTransaction) -> Dict[str, Any]:
        ...
        velocity_kmh = (distance / (time_diff / 3600)) if time_diff > 0 else float('inf')
    ...
    def _update_frequent_locations(self, pattern: TravelPattern, location: GeoLocation):
        ...
        for freq_loc in pattern.frequent_locations:
            distance = self._calculate_distance(
                freq_loc.latitude, freq_loc.longitude,
        ...
        if pattern.home_location:
            distance_from_home = self._calculate_distance(
                pattern.home_location.latitude, pattern.home_location.longitude,
                location.latitude, location.longitude
            )
    ...
    def _update_velocity_profile(self, pattern: TravelPattern, transaction: GeoTransaction):
        ...
        if velocities:
            ...
            pattern.velocity_profile['max_km_per_hour'] = (
                alpha * new_max + (1 - alpha) * pattern.velocity_profile['max_km_per_hour']
            )
            
            pattern.velocity_profile['avg_km_per_hour'] = (
                alpha * new_avg + (1 - alpha) * pattern.velocity_profile['avg_km_per_hour']
            )
    
    def _detect_frequent_locations(self, locations: List[GeoLocation]) -> List[GeoLocation]:
        ...
        for group in frequent_groups:
            if len(group) >= 2:  # At least 2 occurrences to be "frequent"
                ...
                closest_location = min(group, key=lambda loc: self._calculate_distance(
    ...
    def _find_nearby_transactions(self, latitude: float, longitude: float,
                                radius_km: float, user_id: str = None) -> List[GeoTransaction]:
    ...
    def _analyze_cluster_risk(self, transactions: List[GeoTransaction]) -> Tuple[str, float]:
    ...
    def _analyze_temporal_pattern(self, transactions: List[GeoTransaction]) -> Dict[str, Any]:
    ...
    def get_area_statistics(self, latitude: float, longitude: float, 
    ...
    def create_risk_zone(self, coordinates: List[Tuple[float, float]], 
    ...
    def export_map_data(self, center_lat: float, center_lon: float,
                       radius_km: float = 10.0) -> Dict[str, Any]:
        """Export data for map visualization"""
        
        # Find data in area
        area_transactions = self._find_nearby_transactions(center_lat, center_lon, radius_km)
        ...
        area_zones = []
        for zone in self.risk_zones.values():
            if zone.geometry.distance(center_point) <= radius_km / 111.0:  # Rough conversion
...
def main():
    ...
    # Detect clusters
    clusters = engine.detect_geographic_clusters('dbscan', {
        'eps_km': 1.0,
        'min_samples': 2,
    ...
    print(f"Map data: {len(map_data['transactions'])} transactions, {len(map_data['clusters'])} clusters")
