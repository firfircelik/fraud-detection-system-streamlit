...
class GraphEmbeddingStorage:
    ...
    async def _create_embedding_tables(self):
        ...
        try:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS node_embeddings_node_id_idx ON node_embeddings(node_id);")
            cursor.execute("CREATE INDEX IF NOT EXISTS node_embeddings_type_idx ON node_embeddings(node_type);")
            cursor.execute("CREATE INDEX IF NOT EXISTS edge_embeddings_source_idx ON edge_embeddings(source_node_id);")
            cursor.execute("CREATE INDEX IF NOT EXISTS edge_embeddings_target_idx ON edge_embeddings(target_node_id);")
    ...
    async def get_node_embedding(self, node_id: str, model_version: str) -> Optional[NodeEmbedding]:
    ...
    async def find_similar_nodes(self, query_vector: np.ndarray, 
                                node_type: str = None,
    ...
    async def random_walk_sampling(self, start_node_id: str, 
                                  walk_length: int = 10,
                                  num_walks: int = 100) -> List[List[str]]:
        ...
        try:
            ...
            for _ in range(num_walks):
                ...
                for step in range(walk_length - 1):
                    ...
                    weights = [graph[current_node][neighbor].get('weight', 1.0) 
    ...
    async def neighborhood_sampling(self, center_node_id: str,
                                   num_hops: int = 2,
                                   max_nodes_per_hop: int = 10) -> GraphSample:
        ...
        try:
            ...
            for hop in range(num_hops):
                ...
                for node in current_level:
                    ...
                    if len(neighbors) > max_nodes_per_hop:
                        ...
                        neighbor_weights = [(n, graph[node][n].get('weight', 1.0)) 
                                          for n in neighbors]
            ...
            logger.info(f"Created neighborhood sample: {len(sampled_nodes)} nodes, {len(sampled_edges)} edges")
            return sample
    ...
    async def stratified_sampling(self, node_types: List[str],
                                 samples_per_type: int = 100) -> List[GraphSample]:
    ...
    async def extract_graph_features(self, node_id: str, 
                                   feature_methods: List[str] = None) -> GNNFeatureVector:
        ...
        try:
            ...
            if 'degree' in feature_methods:
                ...
                out_degree = graph.out_degree(node_id) if graph.is_directed() else degree
            ...
            if 'local' in feature_methods:
                ...
                feature_names.extend(['avg_neighbor_degree', 'max_neighbor_degree', 'min_neighbor_degree'])
    ...
    async def _extract_node_features(self, node_ids: List[str]) -> Dict[str, np.ndarray]:
    ...
    async def _extract_edge_features(self, edges: List[Tuple[str, str, str]]) -> Dict[str, np.ndarray]:
    ...
    async def _cache_node_embedding(self, embedding: NodeEmbedding):
        ...
        try:
            cache_key = f"{self.cache_prefix}node:{embedding.node_id}:{embedding.model_version}"
            embedding_data = {
    ...
    async def _get_cached_node_embedding(self, node_id: str, model_version: str) -> Optional[NodeEmbedding]:
    ...
    async def _store_graph_sample(self, sample: GraphSample):
        ...
        try:
            ...
            subgraph_data = {
                ...
                'node_features': {k: v.tolist() for k, v in sample.node_features.items()},
                'edge_features': {k: v.tolist() for k, v in sample.edge_features.items()},
