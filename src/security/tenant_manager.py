...
class TenantManager:
    ...
    async def create_security_tables(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            # Create indexes
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_audit_trail_tenant_time ON audit_trail (tenant_id, timestamp DESC)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_security_events_tenant_time ON security_events (tenant_id, created_at DESC)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_user_sessions_active ON user_sessions (user_id, is_active, last_activity DESC)")
    
    async def setup_row_level_security(self):
        """Set up Row Level Security policies"""
        async with self.pool.acquire() as conn:
            # Enable RLS on main tables
            tables_with_rls = ['transactions', 'users', 'merchants', 'fraud_alerts', 'ml_features']
    ...
    async def create_tenant(self, tenant_data: Dict[str, Any]) -> Tenant:
        ...
        async with self.pool.acquire() as conn:
            ...
            """, (
                tenant.tenant_id, tenant.tenant_name, tenant.subscription_tier,
                tenant.resource_limits, tenant.configuration, tenant.is_active,
    ...
    async def create_user(self, user_data: Dict[str, Any]) -> User:
        ...
        async with self.pool.acquire() as conn:
            ...
            """, (
                ...
                user.password_hash, [p.value for p in user.permissions],
        ...
        await self.log_audit_event(
            ...
            new_values={'user_id': user.user_id, 'username': user.username, 'email': user.email}
        )
    ...
    async def authenticate_user(self, username: str, password: str, ip_address: str, user_agent: str) -> Optional[Dict[str, Any]]:
        ...
        async with self.pool.acquire() as conn:
            ...
            if user_row['locked_until'] and user_row['locked_until'] > datetime.now(timezone.utc):
                await self.log_security_event(
            ...
            if not bcrypt.checkpw(password.encode('utf-8'), user_row['password_hash'].encode('utf-8')):
                ...
                await conn.execute("""
                    ...
                    WHERE user_id = $3
                """, failed_attempts, locked_until, user_row['user_id'])
            ...
            await conn.execute("""
                ...
                VALUES ($1, $2, $3, $4, $5, $6)
            """, session_id, user_row['user_id'], user_row['tenant_id'], ip_address, user_agent, expires_at)
    ...
    async def log_audit_event(self, tenant_id: str = None, user_id: str = None, session_id: str = None,
        ...
        async with self.pool.acquire() as conn:
            ...
            """, (
                tenant_id, user_id, session_id, action, resource_type, resource_id,
                old_values, new_values, ip_address, user_agent, metadata or {}
            ))
    
    async def log_security_event(self, event_type: str, severity: str, title: str, description: str,
                               tenant_id: str = None, user_id: str = None, ip_address: str = None,
                               user_agent: str = None, event_data: Dict = None):
        """Log security event"""
        async with self.pool.acquire() as conn:
            await conn.execute("""
            ...
            """, (
                ...
                user_id, ip_address, user_agent, event_data or {}
            ))
    
    async def check_resource_limits(self, tenant_id: str, resource_type: str, current_usage: int) -> bool:
...
async def main():
    ...
    try:
        ...
        user = await tenant_manager.create_user({
            ...
            'permissions': ['admin_access', 'read_transactions', 'write_transactions']
        })
