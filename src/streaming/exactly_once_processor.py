...
class ExactlyOnceProcessor:
    ...
    async def begin_transaction(self, data: Dict[str, Any], 
                               participant_ids: List[str],
        ...
        await self._store_idempotency_key(idempotency_key, {'transaction_id': transaction_id})
        
        self.metrics['transactions_started'] += 1
        logger.info(f"Started transaction {transaction_id} with {len(participant_ids)} participants")
        ...
        for participant_id in transaction.participant_ids:
            try:
                ...
                prepare_success = await self._simulate_participant_prepare(
                    participant_id, transaction_id, transaction.metadata['original_data']
                )
    ...
    async def commit_transaction(self, transaction_id: str) -> bool:
        ...
        try:
            ...
            if all_committed:
                ...
                await self._handle_partial_commit_failure(transaction_id, commit_results)
                return False
    ...
    async def _align_operator_checkpoint(self, operator_id: str, barrier: CheckpointBarrier) -> bool:
        ...
        logger.debug(f"Operator {operator_id} aligned for checkpoint {barrier.checkpoint_id}")
        return True
    ...
    async def _log_transaction_operation(self, transaction_id: str, operation: str,
                                       data: Dict[str, Any], checksum: str):
    ...
    async def _simulate_participant_prepare(self, participant_id: str, 
                                          transaction_id: str, data: Dict[str, Any]) -> bool:
    ...
    async def _simulate_participant_commit(self, participant_id: str, transaction_id: str) -> bool:
    ...
    async def _simulate_participant_abort(self, participant_id: str, transaction_id: str):
        """Simulate participant abort"""
        await asyncio.sleep(0.01)
        logger.debug(f"Participant {participant_id} aborted transaction {transaction_id}")
    ...
    def _is_transaction_expired(self, transaction: ProcessingTransaction) -> bool:
        ...
        return (datetime.now(timezone.utc) - transaction.start_time) > transaction.timeout_duration
    
    async def _handle_partial_commit_failure(self, transaction_id: str, 
        ...
        failed_participants = [
            pid for i, pid in enumerate(transaction.participant_ids)
            if not commit_results[i]
        ]
        
        logger.error(f"Partial commit failure for transaction {transaction_id}. "
    ...
    async def _periodic_checkpoint(self):
        ...
        while self.running:
            try:
                ...
                if (datetime.now(timezone.utc) - self.last_checkpoint) > self.checkpoint_interval:
    ...
    async def _commit_pending_transactions(self):
        ...
        for transaction_id in pending_transactions:
            try:
                ...
                logger.error(f"Error committing pending transaction {transaction_id}: {e}")
...
class ExactlyOnceAPI:
    ...
    async def process_with_exactly_once(self, data: Dict[str, Any], 
