...
class RedisStreamsManager:
    ...
    async def publish_fraud_event(self, event: FraudEvent) -> str:
        ...
        try:
            ...
            stream_id = await self.redis_client.xadd(
                self.streams['fraud_events'], 
                event_data
            )
            
            logger.debug(f"Published fraud event {event.event_id} to stream: {stream_id}")
            return stream_id
    ...
    async def publish_transaction_event(self, transaction_data: Dict[str, Any]) -> str:
        ...
        try:
            stream_id = await self.redis_client.xadd(
                self.streams['transaction_events'],
                transaction_data
            )
    ...
    async def publish_model_prediction(self, prediction_data: Dict[str, Any]) -> str:
        ...
        try:
            stream_id = await self.redis_client.xadd(
                self.streams['model_predictions'],
                prediction_data
            )
    ...
    async def start_consumer(self, stream_name: str, group_name: str, 
                           consumer_name: str, handler: Callable) -> None:
    ...
    async def fraud_event_handler(self, message_id: str, fields: Dict[str, str]):
        ...
        try:
            ...
            logger.info(f"Processing fraud event {message_id}: score={fraud_score}, risk={risk_level}")
    ...
    async def alert_handler(self, message_id: str, fields: Dict[str, str]):
        ...
        try:
            ...
            logger.info(f"Processing alert {message_id}: type={alert_type}, severity={severity}")
    ...
    async def get_consumer_group_info(self, stream_name: str) -> List[StreamConsumerInfo]:
        ...
        try:
            ...
            for group in groups:
                ...
                for consumer in consumers:
                    group_info.append(StreamConsumerInfo(
