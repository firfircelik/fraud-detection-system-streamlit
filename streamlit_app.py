...
def fetch_velocity_anomalies():
    ...
    try:
        response = requests.get(f"{API_BASE_URL}/api/graph/velocity-anomalies", timeout=10)
        if response.status_code == 200:
            return response.json()
        return None
    except Exception as e:
...
def get_ensemble_status():
    ...
    try:
        # Get ensemble status
        ensemble_response = requests.get(f"{API_BASE_URL}/api/ensemble/status", timeout=10)
        ...
        if ensemble_response.status_code == 200 and stats_response.status_code == 200:
            ...
            combined_data = {
                ...
                "models": {},
                "ensemble": {
                    "status": "active" if ensemble_data.get("status") == "success" else "inactive",
...
def search_transactions(query_params):
    ...
    try:
        response = requests.post(
            f"{API_BASE_URL}/api/elasticsearch/search",
            json=query_params,
            timeout=15
        )
        if response.status_code == 200:
...
def process_csv_file(file_data, sample_size=None):
    ...
    try:
        ...
        response = requests.post(f"{API_BASE_URL}/api/transactions/batch", files=files, data=data)
        if response.status_code == 200:
            return response.json()
        return None
    except Exception as e:
...
if not api_status:
    st.error("⚠️ Cannot connect to Backend API. Please make sure the backend service is running.")
    st.info("To start the backend: `cd backend && python -m uvicorn api.main:app --reload`")
    st.stop()
...
with st.sidebar:
    ...
    if api_healthy:
        st.markdown('<span class="status-healthy">🟢 API Online</span>', unsafe_allow_html=True)
    else:
...
"""
                    ...
                    if 'model_metrics' in system_stats:
                        for model_name, metrics in system_stats['model_metrics'].items():
                            accuracy = metrics.get('accuracy', 0)
                            report_content += f"- **{model_name}:** {accuracy:.2%} accuracy\n"
...
if page == "🏠 Dashboard":
    ...
    with st.form("transaction_form"):
        ...
        with col1:
            user_id = st.text_input("👤 User ID", placeholder="User ID veya Merchant ID'den biri gerekli")
            merchant_id = st.text_input("🏪 Merchant ID", placeholder="User ID veya Merchant ID'den biri gerekli")
            amount = st.number_input("💰 Amount (Optional)", min_value=0.01, value=100.0, step=0.01)
            currency = st.selectbox("💱 Currency (Optional)", ["USD", "EUR", "TRY", "GBP"])
        ...
        if submitted:
            ...
            if not user_id_clean and not merchant_id_clean:
                ...
                transaction_data = {
                    ...
                    "merchant_id": merchant_id_clean if merchant_id_clean else "UNKNOWN",
                    "amount": amount,
                    "currency": currency,
                    "transaction_type": transaction_type if transaction_type != "None" else None,
    ...
    if model_status:
        ...
        for model_name, model_info in models.items():
            with st.expander(f"📈 {model_name}"):
                ...
                with col2:
                    st.write(f"**Last Training:** {model_info.get('last_trained', 'Unknown')}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'hourly_patterns' in analytics_data and analytics_data['hourly_patterns']:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    fig_hourly = px.bar(hourly_df, x='hour', y='fraud_rate', 
                                       title='Fraud Rate by Hour of Day',
                                       labels={'fraud_rate': 'Fraud Rate (%)', 'hour': 'Hour'})
                    st.plotly_chart(fig_hourly, use_container_width=True)
                
                with col2:
                    fig_volume = px.bar(hourly_df, x='hour', y='transaction_count',
                                       title='Transaction Volume by Hour',
                                       labels={'transaction_count': 'Transaction Count', 'hour': 'Hour'})
                    st.plotly_chart(fig_volume, use_container_width=True)
            
            # Display risky merchants
            if 'risky_merchants' in analytics_data and analytics_data['risky_merchants']:
                st.subheader("🏪 High-Risk Merchants")
                merchants_df = pd.DataFrame(analytics_data['risky_merchants'])
                
                col1, col2 = st.columns(2)
                with col1:
                    fig_merchants = px.bar(merchants_df.head(10), x='merchant_id', y='avg_fraud_score',
                                          title='Top 10 Risky Merchants by Fraud Score',
                                          labels={'avg_fraud_score': 'Average Fraud Score', 'merchant_id': 'Merchant ID'})
                    fig_merchants.update_xaxes(tickangle=45)
                    st.plotly_chart(fig_merchants, use_container_width=True)
                
                with col2:
                    fig_merchant_volume = px.scatter(merchants_df, x='transaction_count', y='avg_fraud_score',
                                                   size='avg_amount', hover_data=['merchant_id'],
            ...
            if 'risky_users' in analytics_data and analytics_data['risky_users']:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    fig_users = px.bar(users_df.head(10), x='user_id', y='avg_fraud_score',
                                      title='Top 10 Risky Users by Fraud Score',
                                      labels={'avg_fraud_score': 'Average Fraud Score', 'user_id': 'User ID'})
                    fig_users.update_xaxes(tickangle=45)
                    st.plotly_chart(fig_users, use_container_width=True)
                
                with col2:
                    fig_user_activity = px.scatter(users_df, x='transaction_count', y='avg_fraud_score',
                                                  size='avg_amount', hover_data=['user_id'],
            ...
            if 'anomaly_detection' in analytics_data:
                ...
                with col2:
                    st.metric("Velocity Anomalies", anomalies.get('velocity_anomalies', 0))
                with col3:
                    st.metric("Geographic Anomalies", anomalies.get('geographic_anomalies', 0))
            ...
            if 'pattern_analysis' in analytics_data:
                ...
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Burst Transactions", patterns.get('burst_transactions', 0))
                with col2:
    ...
    if fraud_rings_data and 'fraud_rings' in fraud_rings_data:
        ...
        with col2:
            critical_rings = len([r for r in rings if r.get('risk_level') == 'CRITICAL'])
        ...
        for ring in rings:
            ...
            with st.expander(expander_label):
                ...
                with col1:
                    ...
                    st.write(f"**Detection Method:** {ring.get('detection_method', 'Unknown')}")
                
                with col2:
                    st.write(f"**Detection Date:** {ring.get('detection_date', 'Unknown')}")
                ...
                if 'members' in ring and ring['members']:
                    ...
                    if n_members > 1:
                        ...
                        # Add nodes
                        node_text = [f"User {member.get('user_id', 'Unknown')}<br>Risk: {member.get('risk_score', 0):.2f}" for member in members]
                        fig.add_trace(go.Scatter(
                            x=x_coords, y=y_coords,
                            mode='markers+text',
                            marker=dict(
                                size=20,
                                color=[member.get('risk_score', 0.5) for member in members],
                                colorscale='Reds',
                                showscale=True,
                                colorbar=dict(title="Risk Score")
                            ),
                            text=[f"User {member.get('user_id', 'Unknown')}" for member in members],
    ...
    if geo_data:
        ...
        if world_map_data:
            ...
            if locations:
                ...
                if not locations_df.empty:
                    ...
                    colors = [risk_colors.get(loc.get('risk_level', 'LOW'), '#4CAF50') for loc in locations]
                    sizes = [max(10, min(50, loc.get('transaction_count', 10) / 10)) for loc in locations]
                    ...
                    fig.add_trace(go.Scattermapbox(
                        ...
                        text=[f"Location: {loc.get('city', 'Unknown')}<br>" +
                    ...
                    for conn in connections:
                        if 'from_lat' in conn and 'to_lat' in conn:
                            fig.add_trace(go.Scattermapbox(
                                ...
                                mode='lines',
                                line=dict(
                                    width=max(1, min(5, conn.get('transaction_count', 1) / 20)),
                                    color='rgba(255, 0, 0, 0.6)' if conn.get('is_suspicious', False) else 'rgba(0, 100, 255, 0.4)'
                                ),
                                hoverinfo='skip',
                                showlegend=False
                            ))
        ...
        if country_analytics:
            ...
            if not country_df.empty and 'avg_lat' in country_df.columns and 'avg_lng' in country_df.columns:
                ...
                if not map_df.empty:
                    fig = px.scatter_mapbox(
                        ...
                        color='fraud_rate',
                        hover_name='country',
                        hover_data=['total_transactions', 'fraud_transactions', 'total_amount'],
        ...
        if hotspots_data and 'hotspots' in hotspots_data:
            ...
            if hotspots:
                for hotspot in hotspots:
                    ...
                    risk_class = f"alert-{risk_level.lower()}" if risk_level.lower() in ['critical', 'high', 'medium', 'low'] else "alert-medium"
                    ...
                    with st.container():
                        ...
                        with col1:
                            st.write(f"**Location:** {hotspot.get('city', 'Unknown')}, {hotspot.get('state', 'Unknown')}, {hotspot.get('country', 'Unknown')}")
                            st.write(f"**Fraud Count:** {hotspot.get('fraud_count', 0)}")
                            st.write(f"**Total Transactions:** {hotspot.get('total_transactions', 0)}")
                        
                        with col2:
                            st.write(f"**Fraud Rate:** {hotspot.get('fraud_rate', 0):.2f}%")
                            st.write(f"**Risk Level:** {risk_level}")
                            st.write(f"**Total Amount:** ${hotspot.get('fraud_amount', 0):,.2f}")
                ...
                for anomaly in anomalies:
                    ...
                    with st.container():
                        ...
                        with col1:
                            ...
                            st.write(f"**Transaction Count:** {anomaly.get('transaction_count', 0)}")
                        
                        with col2:
                            st.write(f"**Time Window:** {anomaly.get('time_window', 'Unknown')}")
                            st.write(f"**Risk Level:** {risk_level}")
                            st.write(f"**Total Amount:** ${anomaly.get('total_amount', 0):,.2f}")
    ...
    with st.form("search_form"):
        ...
        with col1:
            ...
            search_amount_min = st.number_input("💰 Min Amount", min_value=0.0, value=0.0)
            search_amount_max = st.number_input("💰 Max Amount", min_value=0.0, value=10000.0)
        
        with col2:
            search_merchant = st.text_input("🏪 Merchant ID")
            search_location = st.text_input("📍 Location")
            search_risk_level = st.selectbox("⚠️ Risk Level", ["All", "LOW", "MEDIUM", "HIGH", "CRITICAL"])
        
        with col3:
            search_date_from = st.date_input("📅 From Date", value=datetime.now() - timedelta(days=30))
            search_date_to = st.date_input("📅 To Date", value=datetime.now())
            search_limit = st.number_input("📊 Max Results", min_value=1, max_value=1000, value=100)
        ...
        if search_submitted:
            ...
            if search_results:
                st.subheader(f"📋 Search Results ({len(search_results.get('transactions', []))} found)")
    ...
    with col2:
        memory_usage = metrics_data.get('system_resources', {}).get('memory_usage_percent', 0)
        st.metric("Memory Usage", f"{memory_usage:.1f}%")
    with col3:
        disk_usage = metrics_data.get('system_resources', {}).get('disk_usage_percent', 0)
    ...
    # Configuration tabs
    tab1, tab2, tab3, tab4 = st.tabs(["🔧 General", "🗄️ Database", "🔒 Security", "📊 Monitoring"])
    ...
    with tab1:
        ...
        with col1:
            ...
            st.number_input("Max Concurrent Users", min_value=1, max_value=10000, value=1000)
        ...
        with col2:
            ...
            st.number_input("Rate Limit (req/min)", min_value=1, max_value=10000, value=1000)
    ...
    with tab2:
        ...
        with col1:
            ...
            st.number_input("Connection Pool Size", min_value=1, max_value=100, value=20)
        
        with col2:
            st.text_input("Redis Host", value="localhost")
            st.number_input("Redis Port", min_value=1, max_value=65535, value=6379)
            st.number_input("Redis TTL (seconds)", min_value=60, max_value=86400, value=3600)
    ...
    with tab3:
        ...
        with col1:
            ...
            st.number_input("Token Expiry (hours)", min_value=1, max_value=168, value=24)
        ...
        with col2:
            ...
            st.number_input("Account Lockout Duration (min)", min_value=1, max_value=1440, value=30)
    ...
    with tab4:
        ...
        with col1:
            ...
            st.number_input("Metrics Retention (days)", min_value=1, max_value=365, value=30)
    ...
    with col3:
        high_risk_nodes = sum(1 for node in mock_network_data['nodes'] if node['fraud_score'] > 0.7)
        st.metric("High Risk Nodes", high_risk_nodes)
    with col4:
        avg_fraud_score = np.mean([node['fraud_score'] for node in mock_network_data['nodes']])
    ...
    features = ['transaction_amount', 'time_since_last_transaction', 'merchant_risk_score', 
               'user_velocity', 'device_fingerprint', 'location_risk', 'payment_method',
               'transaction_frequency', 'amount_deviation', 'merchant_category']
    importance_scores = np.random.uniform(0.1, 0.9, len(features))
    mock_feature_data = dict(zip(features, importance_scores))
    
    # Feature importance chart
    feature_df = pd.DataFrame(list(mock_feature_data.items()), columns=['Feature', 'Importance'])
    ...
    with col1:
        selection_method = st.selectbox(
            "Selection Method",
            ["Variance Threshold", "Correlation Filter", "Recursive Feature Elimination", 
             "LASSO Selection", "Random Forest Importance"]
        )
        
        threshold = st.slider("Importance Threshold", 0.0, 1.0, 0.5, 0.1)
    
    with col2:
        max_features = st.number_input("Max Features", min_value=1, max_value=len(features), value=10)
        
        if st.button("Apply Feature Selection"):
            selected_features = [f for f, score in mock_feature_data.items() if score >= threshold][:max_features]
    ...
    if uploaded_file is not None:
        ...
        with col1:
            sample_size = st.number_input("Sample Size (0 for full file)", min_value=0, value=1000)
            
        with col2:
            processing_mode = st.selectbox(
                "Processing Mode",
                ["Standard", "High Performance", "Memory Optimized"]
            )
        ...
        if st.button("Process CSV File"):
            with st.spinner("Processing file..."):
                ...
                if sample_size > 0 and len(df) > sample_size:
                    ...
                    st.info(f"Processing sample of {sample_size} rows from {len(df)} total rows")
                else:
                    df_sample = df
                ...
                with col1:
                    st.metric("Total Transactions", processing_results["total_transactions"])
                with col2:
                    st.metric("Fraud Detected", processing_results["fraud_detected"])
                with col3:
                    st.metric("High Risk", processing_results["high_risk"])
                with col4:
                    st.metric("Processing Time", f"{processing_results['processing_time']:.2f}s")
                
                # Fraud distribution chart
                fraud_data = pd.DataFrame({
                    'Category': ['Approved', 'High Risk', 'Fraud'],
                    'Count': [
                        processing_results["total_transactions"] - processing_results["high_risk"] - processing_results["fraud_detected"],
                ...
                if st.button("Download Results"):
                    ...
                    df_sample['risk_level'] = pd.cut(df_sample['fraud_score'], 
                                                   bins=[0, 0.3, 0.7, 1.0], 
    ...
    with col1:
        search_type = st.selectbox(
            "Search Type",
            ["Full Text Search", "Structured Query", "Geo-spatial Search", "Anomaly Detection"]
        )
        ...
        if search_type == "Full Text Search":
            ...
            fields = st.multiselect(
                "Search Fields",
                ["transaction_id", "user_id", "merchant_name", "description", "location"]
            )
    ...
    with col2:
        ...
        sort_by = st.selectbox("Sort By", ["Relevance", "Date", "Amount", "Fraud Score"])
        ...
        if st.button("Execute Search"):
            with st.spinner("Searching..."):
                ...
                fig = px.scatter(results_df, x='amount', y='fraud_score', 
                               size='relevance_score', hover_data=['transaction_id'],
    ...
    security_events = []
    event_types = ['Login Attempt', 'API Access', 'Data Export', 'Configuration Change', 'Suspicious Activity']
    ...
    with col2:
        ...
        fig = px.bar(x=event_type_counts.index, y=event_type_counts.values,
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        status_color = "🟢" if mock_streaming_status["kafka_status"] == "Connected" else "🔴"
        st.metric("Kafka", f"{status_color} {mock_streaming_status['kafka_status']}")
        
        status_color = "🟢" if mock_streaming_status["redis_status"] == "Connected" else "🔴"
        st.metric("Redis", f"{status_color} {mock_streaming_status['redis_status']}")
        
        status_color = "🟢" if mock_streaming_status["elasticsearch_status"] == "Connected" else "🔴"
        st.metric("Elasticsearch", f"{status_color} {mock_streaming_status['elasticsearch_status']}")
    ...
    if st.button("Start Real-time Monitoring"):
        ...
        for i in range(30):  # Run for 30 seconds
            ...
            with placeholder.container():
                ...
                with col3:
                    fraud_rate = current_fraud / current_events if current_events > 0 else 0
            ...
            with chart_placeholder.container():
                fig = px.line(chart_data, x='time', y=['events', 'fraud_events'],
    ...
    with col2:
        ...
        st.number_input("Processing Timeout (ms)", min_value=100, max_value=10000, value=5000)
        st.selectbox("Processing Mode", ["Exactly Once", "At Least Once", "At Most Once"])
