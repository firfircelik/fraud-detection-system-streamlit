...
if page == "🏠 Dashboard":
    ...
    with st.form("transaction_form"):
        ...
        with col1:
            ...
            currency = st.selectbox("💱 Currency", ["USD", "EUR", "TRY", "GBP"])
        ...
        with col2:
            ...
            category = st.selectbox(
                "🏷️ Category",
                ["grocery", "gas_station", "restaurant", "online", "atm", "retail"]
            )
    ...
    # Fetch system statistics
    stats_data = fetch_system_statistics()
    ...
    if stats_data:
        ...
        with col1:
            ...
            if 'trends' in stats_data:
                trends_df = pd.DataFrame(stats_data['trends'])
                fig = px.line(trends_df, x='date', y='fraud_rate', title='Fraud Rate Trends')
        ...
        with col2:
            ...
            if 'model_metrics' in stats_data:
                metrics = stats_data['model_metrics']
                for model_name, metric_data in metrics.items():
                    st.metric(f"{model_name} Accuracy", f"{metric_data.get('accuracy', 0):.2%}")
    else:
    ...
    fraud_rings = fetch_fraud_rings()
    
    if fraud_rings:
        st.subheader("🔍 Detected Fraud Rings")
        
        for ring in fraud_rings.get('rings', []):
            with st.expander(f"Ring ID: {ring.get('id', 'Unknown')} - Risk: {ring.get('risk_level', 'Unknown')}"):
                col1, col2 = st.columns(2)
                
                with col1:
                    st.write(f"**Members:** {len(ring.get('members', []))}")
                    st.write(f"**Total Amount:** ${ring.get('total_amount', 0):,.2f}")
                    st.write(f"**Transactions:** {ring.get('transaction_count', 0)}")
                
                with col2:
                    st.write(f"**Detection Date:** {ring.get('detected_at', 'Unknown')}")
                    st.write(f"**Confidence:** {ring.get('confidence', 0):.2%}")
                    st.write(f"**Status:** {ring.get('status', 'Unknown')}")
                
                # Display network graph if available
                if 'network_data' in ring:
                    st.subheader("🕸️ Network Visualization")
                    # Here you would create a network graph
                    st.info("Network visualization would be displayed here")
    else:
    ...
    if geo_data:
        ...
        if country_analytics:
            ...
            # Create world map if we have location data
            if not country_df.empty and 'avg_lat' in country_df.columns and 'avg_lng' in country_df.columns:
                ...
                if not map_df.empty:
                    fig = px.scatter_mapbox(
                        ...
                        title='Global Fraud Activity by Country',
                        color_continuous_scale='Reds'
                    )
        ...
        # Fraud Hotspots
        fraud_hotspots = geo_data.get('fraud_hotspots', [])
        if fraud_hotspots:
            st.subheader("🔥 Fraud Hotspots")
            hotspots_df = pd.DataFrame(fraud_hotspots)
            st.dataframe(hotspots_df, use_container_width=True)
        else:
            st.info("No fraud hotspots detected")
        
        # Velocity Anomalies
        velocity_anomalies = geo_data.get('velocity_anomalies', [])
        if velocity_anomalies:
            st.subheader("⚡ Velocity Anomalies")
            
            for anomaly in velocity_anomalies:
                with st.expander(f"User {anomaly.get('user_id', 'Unknown')} - {anomaly.get('risk_level', 'Unknown')} Risk"):
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        st.write(f"**Time Difference:** {anomaly.get('time_diff_hours', 0)} hours")
                        st.write(f"**Distance:** {anomaly.get('distance_approx', 0)} km")
                        st.write(f"**Velocity:** {anomaly.get('velocity', 0)} km/h")
                    
                    with col2:
                        current_loc = anomaly.get('current_location', {})
                        previous_loc = anomaly.get('previous_location', {})
                        st.write(f"**Current Location:** {current_loc.get('lat', 'N/A')}, {current_loc.get('lng', 'N/A')}")
                        st.write(f"**Previous Location:** {previous_loc.get('lat', 'N/A')}, {previous_loc.get('lng', 'N/A')}")
        else:
    ...
    with col2:
        ...
        if st.button("Apply Feature Selection"):
            selected_features = [f for f, score in feature_data.items() if score >= threshold][:max_features]
...
st.markdown(
    ...
    <div style='text-align: center; color: #666; padding: 2rem;'>
        ...
        <small>© 2024 Fraud Detection System. All rights reserved.</small>
    </div>
    """,
    unsafe_allow_html=True
)